        ...
        {isOpen && (
          ...
          >
            <div className="flex flex-col space-y-1 p-6 max-w-7xl mx-auto">
              {['من نحن', 'خدماتنا', 'تواصل معنا'].map((item, index) => (
                <button
                  key={index}
                  onClick={() => { 
                  ...
                  className="text-right px-6 py-4 hover:bg-[#4A7554]/5 rounded-xl font-bold text-[#4A7554] transition-colors"
                >
                  {item}
                </button>
              ))}
              ...
              <div className="pt-4 flex flex-col sm:flex-row gap-3">
                {/* زر إنشاء حساب بأنيميشن تكبير بسيط ولحظي */}
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  transition={{ duration: 0.1 }} // وقت الانيميشن لحظي
                ...
                {/* زر تسجيل الدخول مع هوفر كريمي وأنيميشن سريع */}
                <motion.button
                  whileHover={{ scale: 1.02, backgroundColor: creamyHover }}
                  whileTap={{ scale: 0.98 }}
                  transition={{ duration: 0.1 }}
                  onClick={() => navigate("/login")}
                  className="flex-1 px-6 py-4 text-[#4A7554] border border-[#4A7554]/20 bg-white/50 rounded-xl font-bold text-center transition-colors"
                >
                  تسجيل الدخول
                </motion.button>
              </div>
